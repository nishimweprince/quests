...
export default function Home() {
  return <>
    ...
    <section className="ticker-section" aria-label="Illustrative portfolio names"><div className="ticker-track">{[...tickerNames, ...tickerNames].map((name, index) => <span key={`${name}-${index}`}><b>{name.slice(0, 2).toUpperCase()}</b>{name}</span>)}</div></section>
