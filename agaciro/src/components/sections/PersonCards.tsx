...
export function PortraitArt({ person, priority = false }: { person: Person; priority?: boolean }) {
  return <div aria-hidden="true" className="portrait-art"><Image alt="" fill priority={priority} sizes="(max-width: 767px) 100vw, 44vw" src={person.image} /><span>{initials(person.name)}</span></div>;
}
